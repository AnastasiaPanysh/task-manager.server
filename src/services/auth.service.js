...
async function createUser(name, surname, email, pwd) {
  ...
  if (foundUser.length)
    throw new Error(ExceptionType.REG_USER_SAME_LOGIN.message);
...
async function doAuthorization(email, pwd) {
  const foundUser = await getUserByEmailDB(email);
  if (!foundUser.length)
    throw new Error(ExceptionType.AUTH_USER_WITH_EMAIL.message);

  const userPwd = await checkUserByPwdDB(pwd, email);
  if (!(await bcrypt.compare(pwd, userPwd)))
    throw new Error(ExceptionType.AUTH_USER_WITH_PWD.message);
}
